...
DDIR=os.path.join(DIR,"test1")
LDIR=os.path.join(DDIR,"lc")
RDIR=os.path.join(DDIR,"rc")
FDIR=os.path.join(DDIR,"test1")


SDIR=os.path.join(DDIR,"sc")
...
def main():
    ...
    with tf.device("/gpu:0"):
        ...
        with tf.Session(config=config) as sess:
            ...
            time1 = len([file for file in os.listdir(MDIR) if os.path.isdir(os.path.join(MDIR, file))]) -1
            ...
            showpredict(classifier,FDIR)



def showpredict(classifier,dir):
    print(dir)
    for _, dirs, __ in os.walk(dir):
        ...
        for name in dirs:
            ...
            for i in files:
                ...
                img = cv2.imread(CDIR)
                img2 = cv2.resize(img, (101, 101))
                img3 = np.reshape(img2, [1, 101, 101, 3])
                predict = classifier.predict(img3)
                predict = np.reshape(predict, [3])
                predict = predict[::-1]
                print(np.around(predict, 4))
                plt.bar(range(-1, len(predict) - 1), predict, )
                # plt.figure(figsize=(10, 10))

                # img = img.astype(float) / 255
                cv2.namedWindow("camera", 0)
                # cv2.resizeWindow("camera", 900, 800)
